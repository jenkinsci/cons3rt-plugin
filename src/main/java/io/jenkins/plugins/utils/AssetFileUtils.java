...
public class AssetFileUtils {
	...
	public static File getAssetZipFromPath(final FilePath buildWorkspace, FilePath providedPath) throws IOException, InterruptedException {
		...
		if (providedFile.exists()) {
			if (providedFile.isDirectory()) {
				...
				// the directory
				final List<String> assetPropsFiles = Arrays.asList(providedFile.list(assetPropertiesFilter));
				if (assetPropsFiles.isEmpty()) {
					final String message = "Could not find asset.properties file at top level of directory "
							+ providedPath;
					LOGGER.log(Level.WARNING, message);
					throw new IOException(message);
				} else if (assetPropsFiles.size() > 1) {
				...
				//final FilePath zipFilePath = buildWorkspace.createTempFile("cons3rt-jenkins-plugin-asset-", ".zip");
	...
	public static FilePath createWorkingDirectory(final FilePath workspace, final String name) throws IOException, InterruptedException {
		try {
	...
	public static void createAssetPropertiesFile(final FilePath workDir, final AssetBuilder assetBuilder) throws IOException, InterruptedException {
	...
	public static FilePath verifyFileExists(final FilePath workspace, final String pathToFile) throws IOException, InterruptedException {
	...
	public static void copyAssetFile(final ContextLogger log, final FilePath workspace, final String relativePath, final FilePath targetDirectory) throws IOException, InterruptedException {
	...
	public static FilePath findPrebuiltAsset(FilePath workspace, String prebuiltAssetName) throws IOException, InterruptedException, ParseException {
		...
		final List<String> prebuiltAssets = Arrays.asList(workspaceDir.list(prebuiltAssetFilter));
		
		if (prebuiltAssets.isEmpty()) {
			final String message = "Could not find " + cleanedName + " in workspace.";
			LOGGER.log(Level.WARNING, message);
			throw new IOException(message);
		} else {
			LOGGER.log(Level.INFO, "Found " + prebuiltAssets.size()
					+ " prebuilt assets in workspace.");
